...
public class StarTransFormUtil {
     ...
     * @param len     字符串分割长度
     * @param headLen 属性名长度
     * @param addStr  要添加的字符 (</br>,/n .....)
     * @return 格式化后的字符串
     * @author Youdmeng
     * Date 2020-10-15
     **/
    public static String strAddBr(String str, int len, int headLen, String addStr) {
        ...
        for (int i = 0; i <= times; i++) {
            ...
            if (end < length) {
                str = str.substring(0, end) + addStr + str.substring(end, length);
            }
            if (i == 0) { //第二行后添加头长度
                len += headLen;
            }
